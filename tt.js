...
function createStatsDisplayElements(statsWrapperParam) {
    // NOTE: this is a THIRD copy of the stats-display markup, alongside the ones in the initial GUI
    // setup and the GUI-reconstruction path further down the file. It's used as a safety-net rebuild,
    // triggered by a MutationObserver watching for the first DOM change after init. Keep all three in
    // sync. otkStatsDisplay is positioned absolutely, anchored to statsWrapper's left edge (which the
    // other two paths size to the TITLE alone) - this makes the title's own center land on the bar's
    // true center regardless of how wide a stat line gets, with stats hanging left-aligned from
    // wherever the title's left edge ends up, rather than the title and stats mutually resizing a
    // shared box that centers neither of them exactly.
    ...
    // Measure the title's height BEFORE otkStatsDisplay is appended (statsWrapper only contains
    // titleContainer at this point, no padding yet either) - this gives us title's true rendered
    // height to position stats against directly.
    const titleHeightForPositioning = statsWrapper.offsetHeight;
    ...
    `;
    // NOTE: deliberately NOT top:100% here. For an absolutely positioned element, percentage `top`
    // is calculated against the containing block's PADDING box - and statsWrapper gets a paddingBottom
    // set below (to reserve room for vertical-centering purposes upstream). top:100% would therefore
    // be measured against a box that ALREADY includes that reserved padding, pushing stats down by
    // roughly its own height on top of where it should start - the two techniques compound instead of
    // being independent. A fixed pixel top, measured directly from the title, sidesteps this entirely.
    otkStatsDisplay.style.top = (titleHeightForPositioning + 2) + 'px';
    ...
    threadsTrackedStat.style.cssText = `
        ...
        line-height: 13px;
        height: 13px; /* Explicit height too, not just line-height - the actual stat text is built via
                          a nested lineContainer/baseSpan structure (see updateStatLine) that doesn't
                          reliably inherit line-height for sizing. Explicit height is a hard
                          constraint regardless of nested content. */
        overflow: hidden;
    ...
    // Absolutely positioned, so it no longer contributes to statsWrapper's natural height - reserve
    // the actual space it needs so vertical centering upstream still accounts for the full group.
    // NOTE: this used to measure otkStatsDisplay.offsetHeight, but at this point the 4 stat rows are
    // still empty (updateDisplayedStatistics() fills in the actual numbers later, elsewhere) - an
    // empty flex row collapses toward 0 height regardless of line-height, so that measurement was
    // drastically underestimating the reserved space. Use a fixed value computed from the same CSS
    // we control directly instead: 4 stat rows at line-height:13px each, plus the 2px gap above them.
    statsWrapper.style.position = statsWrapper.style.position || 'relative';
    statsWrapper.style.paddingBottom = (2 + 4 * 13) + 'px';
...
document.addEventListener("visibilitychange", () => {
  ...
  if (typeof updateTabTitle === 'function') updateTabTitle();
    ...
    // ===== TEMPORARY DIAGNOSTIC LOGGING (remove once the OOM crashes are root-caused) =====
    // An OOM crash kills the tab - and the console along with it - instantly, with no chance for JS to
    // react or log anything at the moment it happens. console.log is useless here since it dies with
    // the tab. This persists periodic snapshots to localStorage instead, which survives the crash, so
    // after a reload the last several entries show what was happening in the run-up to it (steady
    // climb vs. a sudden spike, which thread/operation was active, etc).
    const OTK_DIAG_LOG_KEY = 'otkDiagLog';
    const OTK_DIAG_LOG_MAX_ENTRIES = 500;

    function otkLogDiagnostic(label) {
        try {
            let totalMessages = 0;
            let threadCount = 0;
            if (typeof messagesByThreadId === 'object' && messagesByThreadId) {
                for (const tid in messagesByThreadId) {
                    threadCount++;
                    const msgs = messagesByThreadId[tid];
            ...
            const entry = {
                ...
                createdBlobUrls: (typeof createdBlobUrls !== 'undefined' && createdBlobUrls) ? createdBlobUrls.size : null,
                videoBlobUrlCache: (typeof videoBlobUrlCache !== 'undefined' && videoBlobUrlCache) ? videoBlobUrlCache.size : null,
                viewerOpen: (typeof otkViewer !== 'undefined' && otkViewer) ? otkViewer.style.display === 'block' : null,
                viewerMode: localStorage.getItem('otkViewerMode') || 'closed',
            };
            // performance.memory is Chrome/Brave-specific (non-standard) but available unflagged -
            // exactly the browser in use here, per earlier conversation.
            if (window.performance && performance.memory) {
                entry.usedMB = Math.round(performance.memory.usedJSHeapSize / 1048576);
                entry.totalMB = Math.round(performance.memory.totalJSHeapSize / 1048576);
                entry.limitMB = Math.round(performance.memory.jsHeapSizeLimit / 1048576);
            }

            let log = [];
            try {
                log = JSON.parse(localStorage.getItem(OTK_DIAG_LOG_KEY)) || [];
            } catch (e) { log = []; }
            log.push(entry);
            if (log.length > OTK_DIAG_LOG_MAX_ENTRIES) {
                log = log.slice(-OTK_DIAG_LOG_MAX_ENTRIES);
            }
            localStorage.setItem(OTK_DIAG_LOG_KEY, JSON.stringify(log));
        } catch (e) {
            // Never let diagnostic logging itself cause a problem.
        }
    }

    // Heartbeat, so we have periodic context even between explicit event-based log points below.
    setInterval(() => otkLogDiagnostic('heartbeat'), 15000);

    // Retrieval helpers, for use from the browser DevTools console after a crash+reload:
    //   otkGetDiagLog()      -> the raw array of entries
    //   otkPrintDiagLog()    -> pretty-prints a table to the console
    //   otkClearDiagLog()    -> wipes the log (e.g. after copying it out, to start fresh)
    // NOTE: assigned to unsafeWindow, not window - this script runs in Tampermonkey's sandbox (it uses
    // @grant values other than none), so plain `window` here is the sandbox's window, not the actual
    // page window the DevTools console operates against. Plain `window` is kept as a fallback for
    // userscript managers/environments where unsafeWindow isn't provided.
    const otkDiagTargetWindow = (typeof unsafeWindow !== 'undefined') ? unsafeWindow : window;
    otkDiagTargetWindow.otkGetDiagLog = () => {
        try { return JSON.parse(localStorage.getItem(OTK_DIAG_LOG_KEY)) || []; } catch (e) { return []; }
    };
    otkDiagTargetWindow.otkPrintDiagLog = () => {
        const log = otkDiagTargetWindow.otkGetDiagLog();
        console.table(log);
        return `${log.length} entries. Copy the table above, or run copy(JSON.stringify(otkGetDiagLog())) to copy raw JSON to your clipboard.`;
    };
    otkDiagTargetWindow.otkClearDiagLog = () => {
        localStorage.removeItem(OTK_DIAG_LOG_KEY);
    ...
    let mediaAttachmentIntersectionObserver = null; // For lazy loading message-attached images/videos
...
let newReplyIds = new Set(JSON.parse(localStorage.getItem('otkNewReplyIds') || '[]'));
    ...
    function setupLoadingScreen() {
        ...
        overlay.style.cssText = `
            position: fixed;
            top: 100px; /* Height of otkGuiWrapper (96px) + border (4px) */
            left: 0;
            width: 100%;
            height: calc(100vh - 100px); /* Full viewport height minus GUI height */
...
function createTweetEmbedElement(tweetId) {
    ...
    if (!otkGuiWrapper) {
        ...
        otkGuiWrapper.style.cssText = `
            ...
            left: 0;
            right: 0;
            height: 100px; /* 96px for GUI + 4px for border - increased from 85px so the title+stats block
                                could have equal padding above/below instead of being flush against
                                the top edge */
            z-index: 9999;
        ...
        otkGui.style.cssText = `
            ...
            box-sizing: border-box;
            display: grid;
            grid-template-columns: 1fr auto 1fr; /* Equal outer columns so the middle one is truly
                                                     centered on the bar, regardless of how much
                                                     wider the button row is than the thread list -
                                                     flex-grow centers on leftover space, which isn't
                                                     the same thing when the two sides are unequal. */
            align-items: stretch;
            user-select: none;
            position: relative;
            z-index: 1;
        ...
        threadDisplayContainer.style.cssText = `
            display: flex;
            flex-direction: column;
            justify-content: flex-start; /* Top-aligned to match centerInfoContainer, so the title
                                             lines up with the top thread row. */
            max-width: 450px;
            min-width: 0;
            grid-column: 1;
        `;
        ...
        // NOTE: this used to use position:absolute + translate(-50%,-50%), then briefly flex-grow:1 -
        // neither actually centers on the bar's true midpoint when the two side panels (thread list
        // vs button row) are unequal widths, since both techniques center on leftover space between
        // the siblings, not the bar itself. otkGui is now a grid with equal 1fr side columns, so this
        // auto-sized middle column is mathematically centered regardless of how lopsided the sides are.
        // Keep this in sync with the equivalent blocks in the GUI-reconstruction path below AND
        // createStatsDisplayElements() near the top of the file (a third, safety-net rebuild path).
        ...
        centerInfoContainer.style.cssText = `
            ...
            flex-direction: column;
            align-items: center;
            justify-content: flex-start; /* Top-aligned to match threadDisplayContainer (also
                                             flex-start), so the title lines up with the top thread
                                             row when 3 threads are shown. */
            padding: 0 10px;
        `;

        // Wrapper for title and stats. The title is the only in-flow child, so this wrapper's width
        // (and therefore its centered position) is driven ONLY by the title - a long stat line no
        // longer drags the title off-center. Stats are positioned absolutely, anchored to this same
        // left edge, so they hang left-aligned with the title regardless of their own width.
        ...
        otkStatsDisplay.style.cssText = `
            ...
            min-width: 200px; /* Reserve space for (+n) */
        `;
        // NOTE: deliberately NOT top:100% here. For an absolutely positioned element, percentage
        // `top` is calculated against the containing block's PADDING box - and statsWrapper gets a
        // paddingBottom set below (to reserve room for vertical-centering purposes upstream). top:100%
        // would therefore be measured against a box that ALREADY includes that reserved padding,
        // pushing stats down by roughly its own height on top of where it should start - the two
        // techniques compound instead of being independent. A fixed pixel top, measured directly from
        // the title once it's in the live DOM (below), sidesteps this entirely.
        ...
        threadsTrackedStat.style.cssText = `
            ...
            justify-content: flex-start;
            line-height: 13px;
            height: 13px; /* Explicit height too, not just line-height - see the comment on the
                              equivalent line in createStatsDisplayElements for why. */
            overflow: hidden;
            color: var(--otk-stats-text-color);
            min-width: 200px; /* Prevent shifting from (+n) */
            white-space: nowrap;
        ...
        // Measure the title's real rendered height now that it's in the live DOM, and position stats
        // directly below it with a fixed pixel value - see the comment on otkStatsDisplay's cssText
        // above for why this can't be top:100% combined with the paddingBottom reservation below.
        otkStatsDisplay.style.top = (titleContainer.offsetHeight + 2) + 'px';
        statsWrapper.appendChild(otkStatsDisplay);

        // otkStatsDisplay is absolutely positioned now (so a wide stat line can't drag the title
        // off-center), which means it no longer contributes to statsWrapper's natural height. Without
        // this, centerInfoContainer's vertical centering would only account for the title's height,
        // visually pushing the whole title+stats group off its intended vertical position. Reserve
        // the actual space stats needs so vertical centering still accounts for the full group.
        // NOTE: this used to measure otkStatsDisplay.offsetHeight, but at this point the 4 stat rows
        // are still empty (updateDisplayedStatistics() fills in the actual numbers later, elsewhere)
        // - an empty flex row collapses toward 0 height regardless of line-height, so that measurement
        // drastically underestimated the reserved space (title sat too low, stats overflowed the
        // bottom). Use a fixed value computed from the same CSS we control directly instead: 4 stat
        // rows at line-height:13px each, plus the 2px gap above them. (updateDisplayedStatistics()
        // re-measures and corrects this once real content is in place - see that function.)
        statsWrapper.style.paddingBottom = (2 + 4 * 13) + 'px';
        ...
        if (document.body.style.paddingTop !== '100px') {
            document.body.style.paddingTop = '100px';
        }
        ...
        if (!otkGui) { // Re-create otkGui if missing
            ...
            otkGui.style.cssText = `
                ...
                box-sizing: border-box;
                display: grid;
                grid-template-columns: 1fr auto 1fr; /* Kept in sync with the initial-setup path above */
                align-items: stretch;
                user-select: none;
            `;
            otkGuiWrapper.appendChild(otkGui);
        ...
        if (!document.getElementById('otk-center-info-container')) {
            // Keep this in sync with the equivalent blocks in the initial GUI setup above AND
            // createStatsDisplayElements() near the top of the file (a third, safety-net rebuild path).
            const centerInfoContainer = document.createElement('div');
            centerInfoContainer.id = 'otk-center-info-container';
            // Apply styles
            centerInfoContainer.style.cssText = `
                ...
                flex-direction: column;
                align-items: center;
                justify-content: flex-start; /* Kept in sync with the initial-setup path above */
                color: white;
                text-align: left;
                padding: 0 10px;
            `;
            ...
            titleAndStatsContainer.id = 'otk-stats-wrapper'; // Was missing - other code (createStatsDisplayElements,
                                                               // the MutationObserver safety net) looks this ID up directly.
            ...
            otkStatsDisplay.style.cssText = `
                ...
                display: flex;
                flex-direction: column;
                align-items: flex-start; /* Kept in sync with the initial-setup path above */
                width: fit-content; /* Make block only as wide as its content */
            `;
            // top is set via JS further down, once titleContainer is in the live DOM and measurable -
            // see the comment on the equivalent block in the initial-setup path for why this can't be
            // top:100% combined with the paddingBottom reservation set below.

            const threadsTrackedStat = document.createElement('div');
            threadsTrackedStat.id = 'otk-threads-tracked-stat';
            threadsTrackedStat.style.cssText = 'display: flex; justify-content: flex-start; line-height: 13px; height: 13px; overflow: hidden;';

            const totalMessagesStat = document.createElement('div');
            totalMessagesStat.id = 'otk-total-messages-stat';
            totalMessagesStat.style.cssText = 'display: flex; justify-content: flex-start; line-height: 13px; height: 13px; overflow: hidden;';

            const localImagesStat = document.createElement('div');
            localImagesStat.id = 'otk-local-images-stat';
            localImagesStat.style.cssText = 'display: flex; justify-content: flex-start; line-height: 13px; height: 13px; overflow: hidden;';

            const localVideosStat = document.createElement('div');
            localVideosStat.id = 'otk-local-videos-stat';
            localVideosStat.style.cssText = 'display: flex; justify-content: flex-start; line-height: 13px; height: 13px; overflow: hidden;';
            ...
            // See the matching comment in the initial GUI setup path above: otkStatsDisplay is
            // absolutely positioned so it no longer contributes to titleAndStatsContainer's natural
            // height, so reserve the actual space it needs for vertical centering to stay correct.
            // Fixed value, not a measurement - see the comment on the equivalent line above for why.
            titleAndStatsContainer.style.paddingBottom = (2 + 4 * 13) + 'px';

            // Now that centerInfoContainer is finally attached above, titleContainer is part of the
            // live, rendered DOM and can be measured - position stats directly below it with a fixed
            // pixel top (not top:100%, which would conflict with the paddingBottom set just above).
            otkStatsDisplay.style.top = (titleContainer.offsetHeight + 2) + 'px';
        }
        ...
        if (!document.getElementById('otk-button-container')) {
            const buttonContainer = document.createElement('div');
            buttonContainer.id = 'otk-button-container';
            // Kept in sync with the initial-setup path's buttonContainer above.
            buttonContainer.style.cssText = `
                display: flex;
                flex-direction: column;
    ...
    // Permanently remove a dropped thread's messages from both in-memory state and IndexedDB.
    // Previously, when a thread was confirmed no longer live (see the liveThreadIds checks in both
    // refresh functions), it was only removed from activeThreads - its message history was left in
    // messagesByThreadId AND in IndexedDB's messagesStore, deliberately ("messages are retained", per
    // the comment at the original call site), with no cap or expiry. But loadMessagesFromDB() uses
    // store.getAll(), which loads EVERY thread ever stored, active or not - so that retained data
    // gets reloaded back into memory on essentially every render, for the entire history of the
    // script's use, not just the current tab session. For threads that cycle frequently (archive and
    // get replaced regularly), this accumulates without bound over time, which fits "fine for a while,
    // then OOM" far better than any single-session cause.
    function pruneDroppedThreadData(threadIds) {
        if (!threadIds || threadIds.length === 0) return;
        consoleLog(`[Prune] Removing ${threadIds.length} dropped thread(s) from memory and DB:`, threadIds);
        ...
        try {
            const transaction = otkMediaDB.transaction(['messagesStore'], 'readwrite');
            transaction.onerror = (event) => {
                consoleError('[Prune] Error removing dropped thread messages from DB:', event.target.error);
            };
            const store = transaction.objectStore('messagesStore');
            for (const threadId of threadIds) {
            ...
            consoleError('[Prune] Exception while removing dropped thread messages from DB:', e);
        }
    }

    let hasSweptHistoricalThreadBacklog = false;

    // One-time sweep for the historical backlog: pruneDroppedThreadData only stops NEW accumulation
    // going forward, from the moment this fix is in place. It doesn't touch whatever's already piled
    // up in IndexedDB from before - potentially the entire history of using this script, not just the
    // current tab session, since dropped threads' messages were never cleaned up until now. This scans
    // every threadId actually in IndexedDB and removes any that aren't in the current (now
    // catalog-validated) activeThreads list.
    //
    // Deliberately NOT run at raw page-load/init time: an earlier version of this idea (a loop over
    // messagesByThreadId, still visible commented-out near where activeThreads gets initialized) was
    // disabled specifically because activeThreads could be unreliable that early in startup, and it
    // was wiping out threads that were actually still active. This version only runs once, called from
    // within the first successful catalog-scan-based refresh (background or manual, whichever happens
    // first) - by that point activeThreads has just been validated against the live 4chan catalog, so
    // it's trustworthy, and the guard flag means this only costs one IndexedDB scan per session.
    function sweepHistoricalThreadBacklog() {
        if (hasSweptHistoricalThreadBacklog || !otkMediaDB) return;
        hasSweptHistoricalThreadBacklog = true;
        ...
        try {
            ...
            request.onsuccess = () => {
                const storedThreadIds = Array.isArray(request.result) ? request.result : [];
                const activeThreadIdsSet = new Set(activeThreads.map(id => Number(id)));
                const staleThreadIds = storedThreadIds.filter(id => !activeThreadIdsSet.has(Number(id)));
                if (staleThreadIds.length > 0) {
                    consoleLog(`[Prune] Historical backlog sweep: found ${staleThreadIds.length} stale thread(s) in DB not in the current active list.`);
                    pruneDroppedThreadData(staleThreadIds);
                } else {
                    consoleLog('[Prune] Historical backlog sweep: nothing to clean up.');
                }
            };
            request.onerror = (event) => {
                consoleError('[Prune] Error during historical backlog sweep:', event.target.error);
            };
        } catch (e) {
            consoleError('[Prune] Exception during historical backlog sweep:', e);
        }
    }
    let activeThreads = [];
    try {
...
function renderThreadList() {
    ...
    let isRenderingMessagesInViewer = false;
    let renderMessagesInViewerQueued = false;
    let renderMessagesInViewerQueuedOptions = {};

    // Thin wrapper providing re-entrancy protection. renderMessagesInViewerInner (below) does a full
    // clear-and-rebuild of shared blob-tracking state (createdBlobUrls, videoBlobUrlCache,
    // renderedMessageIdsInViewer, etc.) every time it runs. This function has at least 10 call sites
    // throughout the file, several of them fire-and-forget (not awaited) - without this guard, two of
    // them landing close together (a background refresh arriving mid-way through a manual one, a
    // double-click, a theme-change re-render overlapping a data refresh) can run concurrently and race
    // on that shared state: one call's cleanup step can wipe out tracking for blobs the other call is
    // mid-creation of, leaving those blobs permanently untracked and therefore permanently unrevocable.
    // That's a real, confirmed leak mechanism, and very plausibly the actual cause of OOM crashes on
    // refresh that persisted even after the append-path-specific fixes, since this function is what
    // actually runs on every refresh while the viewer is in Message Tracker mode (appendNewMessages
    // ToViewer bails out to a full renderMessagesInViewer call in that mode). A second/third/etc. call
    // that arrives while one is already running gets queued instead of racing, and runs once the
    // in-progress one finishes, so the latest data still always ends up rendered.
    async function renderMessagesInViewer(options = {}) {
        if (isRenderingMessagesInViewer) {
            renderMessagesInViewerQueued = true;
            renderMessagesInViewerQueuedOptions = options;
            consoleLog("[renderMessagesInViewer] Already in progress; queued another pass.");
            return;
        }

        isRenderingMessagesInViewer = true;
        try {
            let currentOptions = options;
            do {
                renderMessagesInViewerQueued = false;
                await renderMessagesInViewerInner(currentOptions);
                currentOptions = renderMessagesInViewerQueuedOptions;
            } while (renderMessagesInViewerQueued);
        } finally {
            isRenderingMessagesInViewer = false;
        }
    }

    async function renderMessagesInViewerInner(options = {}) {
    ...
    if (messageLimitEnabled) {
        ...
        mediaIntersectionObserver = new IntersectionObserver(handleIntersection, {
            ...
            rootMargin: '200px 0px 200px 0px', // Symmetric (was bottom-only) so scrolling up also
                                                 // preloads content before it's visible, fixing pop-in
                                                 // in that direction too. Kept at 200px rather than the
                                                 // first attempt's 300px - videos can't be safely
                                                 // unloaded once loaded (see handleMediaAttachment
                                                 // Intersection below), so a smaller margin limits how
                                                 // many get proactively triggered at once.
            threshold: 0.01
        });

        if (mediaAttachmentIntersectionObserver) {
            mediaAttachmentIntersectionObserver.disconnect();
            consoleLog('[LazyLoad] Disconnected previous mediaAttachmentIntersectionObserver.');
        }
        mediaAttachmentIntersectionObserver = new IntersectionObserver(handleMediaAttachmentIntersection, {
            root: messagesContainer,
            rootMargin: '200px 0px 200px 0px', // Same reasoning as above.
            threshold: 0.01
        });
        ...
        for (let i = 0; i < totalMessagesToRender; i++) {
            ...
            if (messageElement) {
                ...
                const mediaWrappers = messageElement.querySelectorAll('.image-wrapper, .video-wrapper');
                mediaWrappers.forEach(wrapper => attachmentMediaWrappers.push(wrapper));
            }
...
// updateDisplayedStatistics(); // Refresh stats display -- MOVED TO AFTER PROMISES
        ...
        Promise.all(mediaLoadPromises).then(() => {
            ...
            attachmentMediaWrappers.forEach(wrapper => mediaAttachmentIntersectionObserver.observe(wrapper));
...
updateDisplayedStatistics(false); // Update stats after all media processing is attempted.
    ...
    async function appendNewMessagesToViewer(newMessages) {
        ...
        // NOTE: these used to read localStorage.getItem('otkMessageLimitEnabled'/'otkMessageLimitValue')
        // directly - but that setting is only ever saved inside the THEME_SETTINGS_KEY JSON blob (via
        // saveThemeSetting), never as a standalone top-level key. Those reads always returned null,
        // silently ignoring whatever limit the user actually configured and falling back to the
        // enabled=true/value=250 defaults every time. Read from the same themeSettings object the
        // other two copies of this logic already use correctly.
        const themeSettingsForLimit = JSON.parse(localStorage.getItem(THEME_SETTINGS_KEY)) || {};
        const messageLimitEnabled = themeSettingsForLimit.otkMessageLimitEnabled !== false;
        const messageLimitValue = parseInt(themeSettingsForLimit.otkMessageLimitValue || '250', 10);
        const potentialPruneCount = Math.max(0, messageElements.length + newMessages.length - messageLimitValue);

        // If this batch alone is bigger than the limit, trim it down to just the newest
        // messageLimitValue messages BEFORE building any DOM for it. Without this, a large refresh
        // batch (a big backlog after being away, or a fast-moving thread) gets fully built into DOM -
        // full embed detection, quote-chain traversal, media wrapper creation, the works - for every
        // message, even though most of them are about to be immediately pruned right back out again a
        // few lines down. That wasted construction is a real peak-memory spike, and large enough
        // batches can OOM the tab before pruning ever gets a chance to clean anything up.
        if (messageLimitEnabled && newMessages.length > messageLimitValue) {
            const skippedCount = newMessages.length - messageLimitValue;
            consoleLog(`[AppendLimit] Refresh batch (${newMessages.length}) exceeds limit (${messageLimitValue}) on its own. Skipping DOM construction for the ${skippedCount} oldest messages in this batch - they'd just be pruned immediately anyway.`);
            newMessages = newMessages.slice(-messageLimitValue);
        }
        ...
        otkLogDiagnostic(`append-before-build-loop-count-${newMessages.length}`);
        let buildLoopIndex = 0;
        for (const message of newMessages) {
            buildLoopIndex++;
            if (buildLoopIndex % 25 === 0) {
                otkLogDiagnostic(`append-build-loop-progress-${buildLoopIndex}-of-${newMessages.length}`);
            }
            const boardForLink = message.board || 'b';
            const threadColor = getThreadColor(message.originalThreadId);
        ...
        otkLogDiagnostic('append-after-build-loop-before-prune');

        const newAttachmentMediaWrappers = newContentDiv.querySelectorAll('.image-wrapper, .video-wrapper');
        newAttachmentMediaWrappers.forEach(wrapper => {
            if (mediaAttachmentIntersectionObserver) {
                mediaAttachmentIntersectionObserver.observe(wrapper);
            }
        });
        ...
        if (messageLimitEnabled) {
            ...
            if (messageElements.length > messageLimitValue) {
                ...
                for (let i = 0; i < numToRemove; i++) {
                    ...
                    // Stop observing media wrappers inside the node we're about to remove, so the
                    // observer doesn't keep accumulating references to detached elements forever.
                    if (mediaAttachmentIntersectionObserver) {
                        const prunedMediaWrappers = messageToRemove.querySelectorAll('.image-wrapper, .video-wrapper');
                        prunedMediaWrappers.forEach(wrapper => mediaAttachmentIntersectionObserver.unobserve(wrapper));
                    }

                    // Revoke any blob: URLs still referenced by media inside this node
                    // before dropping it, otherwise the underlying blob data stays
                    // pinned in memory even though the DOM node is gone (OOM leak).
                    const mediaEls = messageToRemove.querySelectorAll('img[src^="blob:"], video[src^="blob:"], source[src^="blob:"]');
                    mediaEls.forEach(mediaEl => {
                        const blobUrl = mediaEl.src;
                        if (createdBlobUrls.has(blobUrl)) {
                            URL.revokeObjectURL(blobUrl);
                            createdBlobUrls.delete(blobUrl);
                            for (const [hash, cachedUrl] of videoBlobUrlCache) {
        ...
        otkLogDiagnostic('append-after-prune');
...
function _populateAttachmentDivWithMedia(
    ...
    if (isImage || isVideo) {
        ...
        if (isImage) {
            ...
            setImageProperties = (mode, options = {}) => {
                ...
                if (isThumb) {
                    ...
                    // Same reasoning as the video fix above: reserve the real aspect ratio before the
                    // image is actually loaded, so messagesContainer.scrollHeight doesn't undercount
                    // any not-yet-loaded full/original-mode image below the current viewport.
                ...
                // Dimensions above are set immediately regardless (so the layout doesn't jump once
                // the real image arrives). The actual fetch/decode is what we defer: on initial
                // creation we only reserve space, and let the lazy-load observer trigger the real
                // load once this image is about to scroll into view.
                if (skipLoad) {
                    return;
                }
            ...
            // Note: these onload/onerror handlers are NOT wrapped in a mediaLoadPromises-tracked
            // promise. Loading is always deferred to the lazy-load observer now (skipLoad above), so
            // most images won't have img.src set until well after this initial render pass - a
            // blocking promise here would mean Promise.all(mediaLoadPromises) never resolves for any
            // image outside the initial viewport, hanging the render. This matches how video
            // attachments already behaved (they never blocked render finalization either).
            ...
            setImageProperties(defaultToThumbnail ? 'thumb' : 'full', { skipLoad: true });
            mediaWrapper._otkLoadMedia = () => setImageProperties(img.dataset.mode);
            mediaWrapper._otkUnloadMedia = () => {
                // Only blob: URLs are ours to reclaim - if it's still pointing at a network URL (never
                // got cached to a blob), there's nothing of ours to revoke; the browser's own image
                // cache handles that case. dataset.mode is left untouched, so if the user had manually
                // expanded this to full/original size, _otkLoadMedia will restore that same mode (just
                // re-fetching) rather than silently reverting their choice back to thumbnail.
            ...
            img.addEventListener('click', () => {
                ...
                mediaWrapper.dataset.otkMediaLoaded = 'true'; // Avoid a redundant reload if the lazy-load observer fires after this
                setImageProperties(nextMode);
            });
            img.addEventListener('setImageProperties', (e) => {
                mediaWrapper.dataset.otkMediaLoaded = 'true';
            ...
            // Reserve the video's real aspect ratio upfront, before it's actually loaded (loading is
            // deferred until it scrolls near - see mediaWrapper._otkLoadMedia below). Without this, an
            // unloaded <video> with no src/metadata renders at some small browser-default size, so
            // messagesContainer.scrollHeight undercounts any video below the current viewport that
            // hasn't loaded yet - which is why 'scroll to bottom' was landing short of the true bottom
            // whenever a thread had videos further down the page.
            if (message.attachment.w && message.attachment.h) {
                video.style.aspectRatio = `${message.attachment.w} / ${message.attachment.h}`;
            }
            mediaElement = video;
            ...
            // Defer the actual load until this video's wrapper is registered with the lazy-load
            // observer (see attachmentMediaWrappers / mediaAttachmentIntersectionObserver) rather
            // than loading every attached video immediately at render time - that burst-loading was
            // the main cause of the message-viewer OOM crashes.
            mediaWrapper._otkLoadMedia = () => loadFromCache(true);
                ...
                while (currentTextSegment.length > 0) {
                    ...
                    if (earliestMatch) {
                        ...
                        if (earliestMatchIsQuoteLink) {
                            ...
                            quoteSpan.style.margin = "0px 0px 4px 0px"; // Slight gap below, so it
                                                                          // doesn't sit flush against
                                                                          // the text/next quote link
    ...
    if (!isTopLevel && hasTruncatedQuotes(message)) {
        ...
        hoverPlusIcon.addEventListener('click', async (e) => {
            ...
            if (newElement) {
                ...
                // This element was inserted outside the normal render passes (which register new
                // media wrappers with mediaAttachmentIntersectionObserver after appending). Since the
                // user just explicitly clicked to reveal it, load its media immediately rather than
                // leaving it to an observer it was never registered with.
                newElement.querySelectorAll('.image-wrapper, .video-wrapper').forEach(wrapper => {
                    if (wrapper.dataset.otkMediaLoaded !== 'true' && typeof wrapper._otkLoadMedia === 'function') {
                        wrapper._otkLoadMedia();
                        wrapper.dataset.otkMediaLoaded = 'true';
                    }
                });
                        ...
                        try {
                            while (true) {
                                ...
                                if (newElement) {
                                    ...
                                    // Same as the single-quote-reveal handler above: this element was
                                    // inserted outside the normal render passes, so trigger its media
                                    // loaders directly rather than leaving them unregistered forever.
                                    newElement.querySelectorAll('.image-wrapper, .video-wrapper').forEach(wrapper => {
                                        if (wrapper.dataset.otkMediaLoaded !== 'true' && typeof wrapper._otkLoadMedia === 'function') {
                                            wrapper._otkLoadMedia();
                                            wrapper.dataset.otkMediaLoaded = 'true';
...
async function backgroundRefreshThreadsAndMessages(options = {}) { // Added options parameter
        otkLogDiagnostic('bg-refresh-start');
        loadUserPostIds();
        ...
        try {
            ...
            const previousActiveThreadsArray = [...activeThreads];
            ...
            if (threadsBeforePruning > threadsAfterPruning) {
                const droppedThreadIds = previousActiveThreadsArray.filter(id => !liveThreadIds.has(id));
                consoleLog(`[BG] Pruned ${threadsBeforePruning - threadsAfterPruning} non-live threads from the active list.`);
                pruneDroppedThreadData(droppedThreadIds);
            }
            sweepHistoricalThreadBacklog(); // No-op after the first successful run - see its own comment.
            ...
            for (const threadId of activeThreads) {
                ...
                try {
                    otkLogDiagnostic(`bg-fetch-thread-${threadId}-start`);
                    const newMessagesResult = await fetchThreadMessages(threadId);
                    otkLogDiagnostic(`bg-fetch-thread-${threadId}-end`);
            ...
            if (viewerIsOpen && !skipViewerUpdate && newMessages.length > 0) {
                ...
                if (autoLoad && !document.hidden) {
                    otkLogDiagnostic(`bg-appendToViewer-start-count-${newMessages.length}`);
                    appendNewMessagesToViewer(newMessages).then(() => otkLogDiagnostic('bg-appendToViewer-end'));
                } else {
            ...
            otkLogDiagnostic('bg-refresh-end');

        } catch (error) {
            otkLogDiagnostic('bg-refresh-error');
    ...
    async function refreshThreadsAndMessages(options = {}) { // Manual Refresh / Called by Clear
        otkLogDiagnostic('manual-refresh-start');
        loadUserPostIds();
        messagesByThreadId = await loadMessagesFromDB(); // Ensure in-memory is synced with DB
        otkLogDiagnostic('manual-refresh-after-loadMessagesFromDB');
        ...
        let newMessagesToAppend = [];
            ...
            for (const threadId of threadsToFetch) {
                ...
                try {
                    otkLogDiagnostic(`manual-fetch-thread-${threadId}-start`);
                    const result = await fetchThreadMessages(threadId); // fetchThreadMessages is already async
                    otkLogDiagnostic(`manual-fetch-thread-${threadId}-end`);
    ...
    localStorage.setItem(THREADS_KEY, JSON.stringify(activeThreads)); // activeThreads is already updated by catalog sync
        ...
        if (!skipViewerUpdate && viewerIsOpen) {
            ...
            otkLogDiagnostic(`manual-appendToViewer-start-count-${finalMessagesToAppend.length}`);
            await appendNewMessagesToViewer(finalMessagesToAppend);
            otkLogDiagnostic('manual-appendToViewer-end');
        } else {
            ...
            otkLogDiagnostic('manual-refresh-error');
            ...
            otkLogDiagnostic('manual-refresh-end');
            if (!isChildCall) {
    ...
    async function clearAndRefresh() {
        ...
        if (viewerWasOpen) {
            ...
            // Revoke all tracked blob URLs BEFORE destroying the DOM nodes that reference them.
            // innerHTML = '' below destroys the img/video elements, but does nothing to the
            // underlying blob data those elements' src attributes point to - without this, every
            // Restart Tracker click while media was loaded permanently leaked all of it (the blob
            // data stays allocated forever, since nothing else ever gets a chance to revoke it once
            // its DOM element and tracking-set entry are both gone).
            for (const url of createdBlobUrls) {
                URL.revokeObjectURL(url);
            }
            createdBlobUrls.clear();
...
const scrollButtonContainer = document.getElementById('otk-scroll-button-container');
    ...
    function toggleViewer() {
        ...
        if (currentMode === 'closed') {
            ...
            // Actually tear the rendered content down, rather than just hiding it. Without this, the
            // entire message-list DOM (every message div, every event listener) and both
            // IntersectionObservers stayed fully alive in memory - just invisible - for as long as the
            // viewer stayed "closed". Message Tracker being closed should mean it isn't retaining
            // anything beyond the raw message data (which stays cached in messagesByThreadId/IndexedDB,
            // as it should - only the rendered presentation gets torn down here).
            otkViewer.innerHTML = '';
            renderedMessageIdsInViewer.clear();
            uniqueImageViewerHashes.clear();
            viewerTopLevelAttachedVideoHashes.clear();
            ...
            if (mediaAttachmentIntersectionObserver) {
                mediaAttachmentIntersectionObserver.disconnect();
                mediaAttachmentIntersectionObserver = null;
            }
    ...
    function updateDisplayedStatistics(isBackgroundUpdate = false) {
        ...
        // Re-measure the actual rendered height now that the rows have real content, and correct the
        // vertical-centering reservation on statsWrapper to match. The fixed value applied at GUI
        // creation time was only ever a placeholder for the brief window before this function first
        // runs - it was based on the row's own line-height, but the real content (built above via
        // updateStatLine, which wraps each row in a nested lineContainer/baseSpan) doesn't reliably
        // inherit that for sizing purposes, so the placeholder didn't match true rendered height. This
        // measures the actual result directly instead of predicting it, and re-applies on every call,
        // so it stays correct even if stat text ever wraps to more lines than expected.
        const statsWrapperForHeightFix = document.getElementById('otk-stats-wrapper');
        const otkStatsDisplayForHeightFix = document.getElementById('otk-stats-display');
        if (statsWrapperForHeightFix && otkStatsDisplayForHeightFix) {
            statsWrapperForHeightFix.style.paddingBottom = (otkStatsDisplayForHeightFix.offsetHeight + 2) + 'px';
        }
    }
...
// --- Dedicated Intersection Handler for Message-Attached Images/Videos ---
// Previously, every image/video attached to a message had its blob URL created immediately and
// unconditionally at DOM-creation time (see _populateAttachmentDivWithMedia), regardless of whether
// it was anywhere near the viewport. For a big thread that meant loading/decoding every attached
// image and video all at once - a burst big enough to OOM the tab, especially with videos. This
// callback defers that work: _populateAttachmentDivWithMedia now stores a loader function on each
// media wrapper instead of loading immediately, and registers the wrapper here; the loader runs once,
// the first time the wrapper scrolls near the viewport.
//
// Images additionally get unloaded once they scroll back out of the (now wider, symmetric) lookahead
// margin - this is safe for images specifically because each one gets its own independently-created
// blob URL (createdBlobUrls), never shared or reused elsewhere. Videos still only load once and are
// never unloaded: video blobs ARE cached/shared globally by filehash (videoBlobUrlCache), so the same
// video posted in multiple messages shares one blob URL - revoking it when one instance scrolls away
// could break a different, still-visible instance of the same video. True video unload would need
// per-element reference counting on that shared cache to be done safely.
function handleMediaAttachmentIntersection(entries, observerInstance) {
    entries.forEach(entry => {
        const wrapper = entry.target;
        const isImageWrapper = wrapper.classList.contains('image-wrapper');
        ...
        if (entry.isIntersecting) {
            ...
            if (!isImageWrapper) {
                // Videos: load once, keep watching stops here (matches the embed observer's pattern).
                observerInstance.unobserve(wrapper);
            }
            // Images: keep observing so we get notified when they scroll back out (see below).
        } else if (isImageWrapper) {
            if (wrapper.dataset.otkMediaLoaded === 'true' && typeof wrapper._otkUnloadMedia === 'function') {
                wrapper._otkUnloadMedia();
                wrapper.dataset.otkMediaLoaded = 'false';
            }
        }
    });
}
...
let isForcingViewerRerender = false;
let viewerRerenderQueued = false;
...
async function forceViewerRerenderAfterThemeChange() {
    ...
    if (isForcingViewerRerender) {
        // A render is already in progress. Starting a second one concurrently would race on shared
        // blob-tracking state (createdBlobUrls/videoBlobUrlCache) - one call's cleanup step can wipe
        // out URLs another call is mid-creation of, leaving those blobs permanently untracked and
        // therefore never revocable (a real, confirmed memory leak mechanism). None of this
        // function's callers await it, so this guard is what actually prevents the overlap. Queue
        // one more pass instead, so the latest settings still get picked up once this one finishes.
        viewerRerenderQueued = true;
        consoleLog("Viewer re-render already in progress; queued another pass.");
        return;
    }

    isForcingViewerRerender = true;
    try {
        do {
            viewerRerenderQueued = false;
        ...
        } while (viewerRerenderQueued);
    } finally {
        isForcingViewerRerender = false;
    }
}
...
async function applyMainTheme() {
    ...
    // Note: this used to also check a GM-storage "main theme" preset (otkMainTheme), but nothing in
    // the script ever wrote to that key, so the lookup could never succeed. Removed the dead branch;
    // theme falls through to the localStorage default handled elsewhere.
    consoleLog('[Theme] No active theme settings found. Using localStorage default.');
}
...
function applyThemeSettings(options = {}) {
    ...
    function enablePipMode() {
        ...
        if (!pipBackground) {
            ...
            pipBackground.style.top = '100px';
            pipBackground.style.left = '50vw';
            pipBackground.style.width = '50vw';
            pipBackground.style.height = 'calc(100% - 100px)';
...
function createThemeOptionRow(options) {
    ...
    function setupOptionsWindow() {
        ...
        function resetAllMainOptionsToDefault(promptUser = true) {
            ...
            // Apply theme changes
            applyThemeSettings(); // Already triggers a viewer re-render internally (forceRerender
                                   // defaults to true) - no need to call forceViewerRerenderAfter
                                   // ThemeChange() again here, that was a guaranteed duplicate call.
            applyScrollButtonPosition();
...
function setupScrollButtons() {
    ...
    if (scrollBottomButton) {
        scrollBottomButton.addEventListener('click', () => {
            ...
            // Chase the target for a short window after the initial scroll. The aspect-ratio
            // reservations on images/video (see _populateAttachmentDivWithMedia) mean scrollHeight
            // should already be accurate before anything loads, but this is a defensive fallback in
            // case something else still resizes after load (a missing w/h on an attachment, an embed
            // that changes size once it actually loads, etc.) - without it, 'scroll to bottom' could
            // land just short of the true bottom whenever content below grows after the initial jump.
            let chaseCount = 0;
            const chaseInterval = setInterval(() => {
                chaseCount++;
                const target = messagesContainer.scrollHeight - messagesContainer.clientHeight;
                if (messagesContainer.scrollTop < target) {
                    messagesContainer.scrollTop = target;
                }
                if (chaseCount >= 10) { // ~2 seconds at 200ms - long enough for lazy-loaded content
                                         // just below the fold to finish resizing, short enough to
                                         // not fight the user if they start scrolling elsewhere.
    ...
    // --- Settings export: keys, not settings ---
    // Every localStorage key the script uses is prefixed 'otk'. Rather than hand-maintain a
    // whitelist of which ones count as "settings" (which has already gone stale once — see the
    // git history/comments this replaced), we export every 'otk'-prefixed key EXCEPT the ones
    // below, which are cached data or transient session-state rather than user configuration:
    const SETTINGS_EXPORT_EXCLUDED_KEYS = new Set([
        MESSAGES_KEY,             // otkMessagesByThreadId - cached message data, can be tens/hundreds of MB
        TWEET_CACHE_KEY,          // otkTweetCache - regenerable cache of tweet embed HTML
        SEEN_EMBED_URL_IDS_KEY,   // otkSeenEmbedUrlIds - internal stats-tracking set, not user-facing
        LOCAL_IMAGE_COUNT_KEY,    // otkLocalImageCount - recomputed counter
        LOCAL_VIDEO_COUNT_KEY,    // otkLocalVideoCount - recomputed counter
        'otkNextUpdateTimestamp', // scheduling timestamp - restoring a stale one would be actively wrong
        'otkNewReplyIds',         // transient per-session state
        VIEWER_OPEN_KEY,          // otkViewerOpen - transient per-session state
        DROPPED_THREADS_KEY,      // otkDroppedThreadIds - legacy one-time-migration leftover
        'otkFilterRules',         // legacy pre-V2 filter rules; superseded by FILTER_RULES_V2_KEY and self-deletes on migration
    ]);

    function getExportableSettingsKeys() {
        return Object.keys(localStorage).filter(key => key.startsWith('otk') && !SETTINGS_EXPORT_EXCLUDED_KEYS.has(key));
    }
    ...
    saveToSystemButton.addEventListener('click', async () => {
        ...
        const allSettings = {};
        const keysToExport = getExportableSettingsKeys();
    ...
    panel.appendChild(loadButtonRow);
        ...
        main().catch(error => {
            ...
            if (centerInfo) {
                consoleLog('[Final Check] Computed grid-column for centerInfoContainer:', window.getComputedStyle(centerInfo).gridColumn);
            } else {
